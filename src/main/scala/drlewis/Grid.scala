...
class Grid {
    ...
    private var _elements:Seq[Element] = for (i <- 0 until 8; j <- 6 until 16; if math.random() < odds) yield {new Virus(i, j, ColorOption.randomColor)}
    ...
    def update(delay: Double):Unit = {
        ...
        if (moveDelay >= moveInterval){
            if (keysHeld(KeyCode.Up)) _currentPill = currentPill.move(-1, 0, isClear)
            if (keysHeld(KeyCode.Down)) _currentPill = currentPill.move(1, 0, isClear)
            moveDelay = 0.0
        }
